 ...
 * <RelativeLayout xmlns:android="http://schemas.android.com/apk/res/android"
   ...
   >
   	<apk.lib.widget.widget.RoundButton
...
public class RoundButton extends View {
	...
	private void init(AttributeSet attrs, int defStyle) {
		...
		if (attrs != null) {
			...
			textColor=a.getColor(R.styleable.RoundButton_textColor, 0xffffffff);
			...
			highlightMode = HighlightMode.getValue(a.getInt(
					R.styleable.RoundButton_highlightMode,
					HighlightMode.None.ordinal()));
		...
		init(image, bgcolor, text,textColor);
	}

	public void init(Drawable image, int bgcolor, String text,int textColor) {
		this.image = image;
	...
	protected void onDraw(Canvas canvas) {
		RectF bounds = new RectF(0, 0,getMeasuredWidth(),getMeasuredHeight());
		...
		float h_dst = ((image != null) ? image.getMinimumHeight()
				+ ((text.length() > 0) ? spacing : 0) : 0)
				+ textBounds.height();

		float xd = (bounds.width() - ((image != null) ? image.getMinimumWidth()
				: 0)) / 2;
		float yd = (bounds.height() - h_dst) / 2;

		if (image != null) {
			image.setBounds((int) xd, (int) yd,
					(int) (xd + image.getMinimumWidth()),
					(int) (yd + image.getMinimumHeight()));
			image.draw(canvas);
		}

		float xt = (bounds.width() - 0 * textBounds.width()) / 2;
		float yt = yd
				+ ((image != null) ? image.getMinimumHeight()
						+ ((text.length() > 0) ? spacing : 0) : textBounds
						.height());// + textBounds.height();
		canvas.drawText(text, xt, yt, textPaint);

		if (checked && checkable && checkedImage != null) {
			checkedImage.setBounds(
					(int) (bounds.width() - checkedImage.getMinimumWidth()),
					(int) (bounds.height() - checkedImage.getMinimumHeight()),
	...
	public void disableButton(int textColor,int bgColor){
		setEnabled(false);
		setTextColor(textColor);
		setBgColor(bgColor);
	}
	
	public void enableButton(int textColor,int bgColor){
		setEnabled(true);
		setTextColor(textColor);
		setBgColor(bgColor);
	}
