...
public class ImageLoader {
    ...
    public void drain(final int sequenceNumber){
    	...
    	Iterator<String> i = this.mInFlightRequests.keySet().iterator();
    	while (i.hasNext()) {
    		String key=i.next();
    		if ((((BatchedImageRequest)this.mInFlightRequests.get(key)).mRequest == null) 
    				|| (((BatchedImageRequest)this.mInFlightRequests.get(key)).mRequest.getSequence() < sequenceNumber)){
    	...
    	while (ri.hasNext()) {
    		String key = (String)ri.next();
    			this.mInFlightRequests.remove(key);
    	}
    }
}
